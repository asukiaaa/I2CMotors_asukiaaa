...
#define DEVICE_NAME "M5StackCar"
#define MOTORS_ADDRESS 0b01010010
#define UUID_SERVICE "00ff"
#define UUID_CHARA "ff01"
I2cMotors_asukiaaa motors(MOTORS_ADDRESS);

int speedL = 0, speedR = 0;
int convertDataToSpeed(uint8_t data) { return ((int)data - 0xff / 2) * 2; }
...
class CharacteristicCallbacks : public NimBLECharacteristicCallbacks {
  ...
  void onWrite(NimBLECharacteristic* pCharacteristic) {
    ...
    Serial.println(pCharacteristic->getValue().c_str());
    auto len = pCharacteristic->getValue().length();
    ...
    Serial.println();
    if (len >= 2) {
      speedL = convertDataToSpeed(data[1]);
      speedR = convertDataToSpeed(data[2]);
    }
  };
...
void setup() {
   ...
   *  0x2904 a NimBLE2904 class is created with the correct properties and
   * sizes. However we must cast the returned reference to the correct type as
   * the method only returns a pointer to the base NimBLEDescriptor class.
   */
  NimBLE2904* pBeef2904 =
      (NimBLE2904*)pBeefCharacteristic->createDescriptor("2904");
  pBeef2904->setFormat(NimBLE2904::FORMAT_UTF8);
  pBeef2904->setCallbacks(&dscCallbacks);

  NimBLEService* pBaadService = pServer->createService(UUID_SERVICE);
  NimBLECharacteristic* pFoodCharacteristic =
      pBaadService->createCharacteristic(
          UUID_CHARA, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE |
                          NIMBLE_PROPERTY::NOTIFY);

  pFoodCharacteristic->setValue("Fries");
  pFoodCharacteristic->setCallbacks(&chrCallbacks);

  /** Note a 0x2902 descriptor MUST NOT be created as NimBLE will create one
   * automatically if notification or indication properties are assigned to a
   * characteristic.
   */

  /** Custom descriptor: Arguments are UUID, Properties, max length in bytes of
   * the value */
...
void loop() {
  ...
  motors.writeSpeed(speedL, speedR);
